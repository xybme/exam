...
export default class Index extends Taro.Component {
  ...
  submitForm() {
    ...
    if (!form.applicant) {
      _showToast({ title: '姓名不能为空', icon: 'none' });
      return;
    }
    if (!isPhoneNum(form.telephone)) {
      _showToast({ title: '请输入正确的手机号', icon: 'none' });
      return;
    }
    console.log(form);
    Taro.fetch({
  ...
  render() {
    const { form } = this.state;
    return <View className="register-page">
        <View className="title">填写资料</View>
        <AtForm>
          <AtInput title="你的姓名" type="text" placeholder="输入姓名" value={form.applicant} onChange={this.handleChange.bind(this, 'applicant')} />
          <AtInput title="你的手机号" type="tel" maxLength="11" placeholder="输入你的手机号" value={form.telephone} onChange={this.handleChange.bind(this, 'telephone')} />
          <View className="range-wrap">
            <View className="text">期望薪资: {form.salaryMin || 0} ~ {form.salaryMax || 0}</View>
            <AtRange sliderStyle="background: #6290E9" value={[0, 0]} min={1000} max={30000} onChange={this.handleChange.bind(this, 'range')} />
          </View>
