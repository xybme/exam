...
export default class Index extends Taro.Component {
  ...
  chooseAnswer(index, subI, questionType) {
    ...
    let result = examList.map(item => {
      ...
      if (item.questionType == 1 && hasCheckAnswer.length) {
        ...
        // 多选
        let ids = [];
        hasCheckAnswer.map(queiyId => {
          ids.push(queiyId.optionId);
        });
        optionId = ids.join();
  ...
  render() {
    ...
    return <View className="exam-page">
        <View className="exam-title">{examInfo.examName}</View>
        <View>
          {examList.map((item, index) => <View className="question-item" key={index}>
              <AtCard isFull title={`${index + 1}、${item.questionName}`} extra={questionTypeArr[item.questionType]}>
                {item.questionType !== 3 && item.options.map((answer, subI) => <View className={answer.isCheck ? 'options checkedbg' : 'options'} key={subI} onClick={this.chooseAnswer.bind(this, index, subI, item.questionType)}>
                ...
                {item.questionType == 3 && <AtTextarea value={item.text} onChange={this.changeTextArea.bind(this, index)} maxLength={999} placeholder="你的回答是..." />}
              </AtCard>
            </View>)}
        </View>
        {examList.length && <AtButton className="center-btn" type="primary" onClick={this.submitExam.bind(this)}>
            提交
          </AtButton>}
        {isEnd && <View className="submit-success">
            <View className="tips">
