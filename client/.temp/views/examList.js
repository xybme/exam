...
export default class Index extends Taro.Component {
  ...
  render() {
    ...
    return <View className="exam-list">
        <BaseMenu title="试卷列表" />
        <AtList>
          {examList.map((item, index) => <View className="item-wrap" onClick={this.showQrCode.bind(this, item.examId)} key={index}>
              <AtListItem title={item.examName} note={item.describe}
          // extraText={item.questionIds}
          />
              <View onClick={this.updataExam.bind(this, item)} className="update-btn">
                修改
              </View>
            </View>)}
        </AtList>
