...
export default class Index extends Taro.Component {
  ...
  state = {
    ...
    positonNameArr: [],
    answerArr: [{ optionName: '', isRight: 0 }, { optionName: '', isRight: 0 }],
    questionForm: {
      questionName: '',
      questionType: 1,
      positionId: '',
    ...
    if (noOptionNameArr[0]) {
      ...
      _showToast({ title: `请输入${answerLabel[index]}答案`, icon: 'none' });
      return;
    }
    if (!rightArr.length) {
      _showToast({ title: '至少要一个正确答案', icon: 'none' });
      return;
    }
  ...
  savaExamCfg() {
    ...
    if (!questionForm.questionName || !questionForm.positionId) {
      _showToast({ title: '题目名或所属职位没填选', icon: 'none' });
      return;
    }
    // 不是问答题就调校验
    ...
    Taro.fetch({
      ...
      _showToast({ title: res.message || '新增问题成功', icon: 'success' });
    });
  }
  ...
  swipeActionHancle(obj) {
    ...
    if (obj.text === '删除') {
      if (answerArr.length <= 2) {
        _showToast({ title: '至少要两个答案', icon: 'none' });
        return;
      }
      this.deleteAnswer();
    } else if (obj.text === '新增') {
      if (answerArr.length >= 7) {
        _showToast({ title: '最多七个答案', icon: 'none' });
        return;
      }
      this.addAnswer();
    }
  ...
  render() {
    ...
    return <View className="add-question">
        <BaseMenu title="新增问题" />
        <AtForm>
          <View>
            <Picker mode="selector" range={questionTypeArr} onChange={this.handleChange.bind(this, 'questionType', 'picker')}>
              <View className="text-row">题目类型: {questionTypeArr[questionForm.questionType - 1] || '选择类型'}</View>
            </Picker>
            <Picker mode="selector" range={positonNameArr} onChange={this.handleChange.bind(this, 'positionId', 'picker')}>
              <View className="text-row"> 所属职位: {positonNameArr[questionForm.positionId - 1] || '选择职位'}</View>
            </Picker>
          </View>
          <AtTextarea value={questionForm.questionName} onChange={this.handleChange.bind(this, 'questionName', 'textarea')} maxLength={200} placeholder="问题题目是..." />
          {questionForm.questionType !== 3 && answerArr.map((item, index) => <AtSwipeAction autoClose onClick={this.swipeActionHancle.bind(this)} key={index} options={[{ text: '新增', style: { backgroundColor: '#6190E8' } }, { text: '删除', style: { backgroundColor: '#FF4949' } }]}>
                <View className="answer-wrap">
                  <View>{answerLabel[index]}、</View>
                  <AtInput className="input" type="text" placeholder={`输入${answerLabel[index]}答案`} value={item.optionName} onChange={this.changeOptionName.bind(this, index)} />
                  <AtButton className={item.isRight ? 'isright-btn isright-btn-true' : 'isright-btn'} onClick={this.changeRightAnswer.bind(this, index, item.isRight)}>
                    {item.isRight ? '正确' : '错误'}
                  </AtButton>
                </View>
              </AtSwipeAction>)}
