...
export default class Index extends Taro.Component {
  ...
  savaExamCfg() {
    ...
    if (!examForm.examName || !examForm.questionIds) {
      _showToast({ title: '试劵名或试题集不能为空', icon: 'none' });
      return;
    }
    ...
    Taro.fetch({
      ...
      _showToast({
        title: res.message || `${examForm.examId ? '修改' : '新增'}试卷成功`,
        icon: 'success'
      });
    });
  }
  ...
  checkItem(index) {
    ...
    checkItemArr.map(item => {
      examForm.questionIds.push(item.id);
    });
  ...
  render() {
    ...
    return <View className="exam-cfg-page">
        <BaseMenu title={examForm.examId ? '修改试卷' : '新增试卷'} />
        <AtForm>
          <AtInput title="试卷名" type="text" placeholder="试卷名" value={examForm.examName} onChange={this.handleChange.bind(this, 'examName')} />
          <AtInput title="描述" type="text" placeholder="描述" value={examForm.describe} onChange={this.handleChange.bind(this, 'describe')} />
          <View className="questions-pool">
            {this.getNameForId().map((item, index) => <View key={index} className="item" onClick={this.deleteQuestionId.bind(this, index)}>
                {item}
                {item && <Image className="delete-icon" src={DELETE} />}
              </View>)}
          </View>
          {questionArr.map((item, index) => <View className="question-item" key={index}>
                <AtListItem onClick={this.checkItem.bind(this, index)} className={item.isCheck && 'check-item'} title={item.questionName} extraText={questionTypeArr[item.questionType]} />
              </View>)}
          ...
          <View className="bottom-btn-wrap">   
            <AtButton type="primary" className="bottom-btn" size="normal" onClick={this.savaExamCfg.bind(this)}>
